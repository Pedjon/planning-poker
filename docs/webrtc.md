...
Every `HEARTBEAT_INTERVAL_MS` (3s) the transport sends a `ping` frame to each open neighbor (`keepaliveTick`). This is a **keepalive only** - it exercises the link and keeps NAT bindings warm. It does not tear a peer down for missed pings, because browsers throttle timers in backgrounded/idle tabs, so silence is not a reliable death signal; dropping on it falsely evicts inactive participants. Real departures are detected by WebRTC itself: the data channel's `onclose` (fires on close/refresh) and `connectionState` transitioning to `failed`/`closed` (genuine connection failure) both call `removeLink`, which fires `onPeerClose` and triggers coordinator re-election. The trade-off is that a truly-gone peer that never fires either event (rare) lingers until ICE eventually fails, rather than being evicted on a fixed timer. Timing constants live in [iceConfig.js](../js/adapters/transport/iceConfig.js).
