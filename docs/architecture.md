...
- [adapters/transport/](../js/adapters/transport/) - the **Transport** port over WebRTC, split for single-responsibility:
  ...
  - `WebRtcTransport.js` - `initHost`/`initJoin` and the data-channel wiring.
...
| Transport (host) | `initHost(handlers) -> { broadcast, acceptJoinRequest }` | `WebRtcTransport` |
| Transport (join) | `initJoin(handlers) -> { createRequest, acceptAnswer, send }` | `WebRtcTransport` |
| UI | `render(vm)`, `setStatus`, `goTo`, `copy` | `UiAdapter` |

`handlers = { onMessage, onOpen, onClose }`.
